...
Uses importlib.metadata to inspect installed distributions without any
...
def host_version(dist_name: str) -> str | None:
    """Return the installed version of dist_name, or None if not found.

    Args:
        dist_name: The distribution name to look up.
...
def parse_requirement_name(requirement: str) -> str | None:
    ...
    Args:
        requirement: A Requires-Dist string from package metadata.

    Returns:
        The bare package name, or None if unparseable.
    """
...
def direct_dependencies(dist_name: str) -> list[tuple[str, str]]:
    ...
    Args:
        dist_name: The distribution name whose deps to enumerate.

    Returns:
        List of (name, version) tuples for installed direct dependencies.
    """
    try:
...
def all_installed() -> list[tuple[str, str]]:
    ...
    Returns:
        List of (name, version) tuples for all installed packages.
    """
    result: list[tuple[str, str]] = []
...
__all__ = ["all_installed", "direct_dependencies", "host_version", "parse_requirement_name"]
